...
+ (NSString *)timeIntervalFromLastTime:(NSString *)lastTime lastTimeFormat:(NSString *)format ToCurrentTime:(NSDate *)currentTime{
    ...
    return [Utilities timeIntervalFromLastTime:lastDate ToCurrentTime:currentTime];
}
...
+ (UIImage *)zoomBlurWithOriginalImage:(UIImage *)image inputCenter:(CGPoint)inputCenter inputAmount:(NSInteger)inputAmount{
    ...
    [filter setValue:inputImage forKey:kCIInputImageKey];
    CIVector *vector = [CIVector vectorWithCGPoint:inputCenter];
...
+ (UIImage *)colorClampWithOriginalImage:(UIImage *)image inputMinComponets:(CGRect)min inputMaxComponets:(CGRect)max{
    ...
    CIVector *minVector = [CIVector vectorWithCGRect:min];
    [filter setValue:minVector forKey:@"inputMinComponents"];
    CIVector *maxVector = [CIVector vectorWithCGRect:max];
...
+ (UIImage *)colorMatrixWithOriginalImage:(UIImage *)image RVector:(CGRect)RVector  GVector:(CGRect)GVector  BVector:(CGRect)BVector  AVector:(CGRect)AVector  BiasVector:(CGRect)BiasVector{
    ...
    CIVector *rVector = [CIVector vectorWithCGRect:RVector];
    CIVector *gVector = [CIVector vectorWithCGRect:GVector];
    CIVector *bVector = [CIVector vectorWithCGRect:BVector];
    CIVector *aVector = [CIVector vectorWithCGRect:AVector];
    CIVector *biasVector = [CIVector vectorWithCGRect:BiasVector];
...
#pragma mark - 创建毛玻璃效果view
+ (UIVisualEffectView *)effectViewWithFrame:(CGRect)frame contentView:(NSArray *)contentViews{
    UIBlurEffect *effect = [UIBlurEffect effectWithStyle:UIBlurEffectStyleLight];
    UIVisualEffectView *effectView = [[UIVisualEffectView alloc] initWithEffect:effect];
    effectView.frame = frame;
    for (int i=0; i<contentViews.count; i++) {
        id obj = contentViews[i];
        if ([obj isKindOfClass:[UIView class]]) {
            [effectView addSubview:(UIView *)obj];
        }
    }
    return effectView;
}
...
+ (UIImage *)compressOriginalImage:(UIImage *)image toMaxFileSizeKBytes:(CGFloat)size{
    NSData *data = [Utilities compressOriginalImage:image toMaxDataSizeKBytes:size];
...
+ (UIImage *)qrCodeImageWithContent:(NSString *)content logo:(UIImage *)logo codeImageSize:(CGFloat)size red:(CGFloat)red green:(CGFloat)green blue:(NSInteger)blue{
    ...
    if (logo != nil) {
        ...
        CGRect logoFrame = CGRectMake((image.size.width-logoWidth)/2,
                                      (image.size.width-logoWidth)/2,
                                      logoWidth,
                                      logoWidth);
